...
module Qtree =
  struct
    ...
    let rect_is_in_lvlh sup_rect rect =
      ...
      if (rect.left <= c.x) and (c.x <= rect.right) then 1
      else if (rect.bottom <= c.y) and (c.y <= rect.top) then 2
      else 0

    let support_contains_rect sup_rect rect =
      (rect.top <= sup_rect.top) and
      (rect.bottom >= sup_rect.bottom) and
      (rect.left >= sup_rect.left) and
    ...
    let insert_rect qtree rect =
      ...
      ;
      (* in the following auxiliary function, sup_rect_qtree is the support rect
         that qtree should have. This is needed to create the qtree when its,
         because the data structures require each node to know its support rect *)
      let aux qtree rect sup_rect_qtree= 
        match qtree with
          | Empty -> match rect_is_in_lvlh sup_rect_qtree rect with
              | 1 -> Q(sup_rect_qtree, [rect], [], Empty, Empty, Empty, Empty)
              | 2 -> Q(sup_rect_qtree, [], [rect], Empty, Empty, Empty, Empty)
              (* In the following case, we could actually watch where rect goes
                 and not call aux on every child *)
              | _ -> let (r1, r2, r3, r4) =
                         (get_rect_squ sup_rect_qtree 1,
                          get_rect_squ sup_rect_qtree 2,
                          get_rect_squ sup_rect_qtree 3,
                          get_rect_squ sup_rect_qtree 4) in
                     Q(sup_rect_qtree, [], [],
                     aux (new_qtree r1) rect r1,
                     aux (new_qtree r2) rect r2,
                     aux (new_qtree r3) rect r3,
                     aux (new_qtree r4) rect r4)
          | Q(r, lv, lh, q1, q2, q3, q4) -> 
            match rect_is_in_lvlh r rect with
              ...
              | _ -> match what_square_contains_rect r rect with
                ...
                | 0 -> (* if rect is not cointaied by qtree, do nothing *)
      in let Q(r, lv, lh, q1, q2, q3, q4) = qtree in aux qtree rect r
